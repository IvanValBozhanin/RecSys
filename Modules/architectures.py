...
class SelectionGNN(nn.Module):
    ...
    def __init__(self,
        ...
        if len(self.dimLayersMLP) > 0: 
            ...
            # weights = weights / (weights.numel() / self.N[0])
            ...
            for l in range(len(dimLayersMLP)-1):
                ...
                fc.append(nn.Linear(dimLayersMLP[l], dimLayersMLP[l+1],
                                    bias = self.bias))
                if l < len(dimLayersMLP)-2: # add non-linearity because we are not at last layer
                    fc.append(self.sigma())
    ...
    def splitForward(self, x):
        ...
        # debug.
        # print(f"x.shape: {x.shape}, self.F[0]: {self.F[0]}, self.N[0]: {self.N[0]}")
        ...
        y = self.GFL(x)
        # Reshape the output
        if self.average: # for graph-level tasks
            yFlat = y.mean(2)
            # yFlat = y.reshape((-1, self.F[-1] * self.N[0]))
            yMLP = self.MLP(yFlat)
        else:
            yFlat = y.permute((0,2,1)).reshape((-1, self.F[-1]))
            yMLP = self.MLP(yFlat).reshape((batchSize, -1))
